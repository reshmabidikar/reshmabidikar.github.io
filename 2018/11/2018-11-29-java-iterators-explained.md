...
\[java\] public static void main(String\[\] args) { List<Integer> list = new ArrayList<Integer>(); for(int i = 0; i &amp;lt; 10; i ++){ list.add(i+2); } Iterator<Integer> itr = list.iterator(); while (itr.hasNext()){ int i = itr.next(); System.out.println("i="+i); } } \[/java\]
